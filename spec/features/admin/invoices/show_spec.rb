...
RSpec.describe 'admin invoices show page' do
  before :each do
    ...
    @potion = @pokemart.items.create!(name: 'Potion', description: 'Recovers 10 HP', unit_price: 200)
    @super_potion = @pokemart.items.create!(name: 'Super Potion', description: 'Recovers 25 HP', unit_price: 500)
    ...
    @invoice_item1 = InvoiceItem.create!(invoice: @invoice1, item: @ultra_ball, quantity: 2, unit_price: 800,
                                         status: 0)
    @invoice_item2 = InvoiceItem.create!(invoice: @invoice1, item: @super_potion, quantity: 2, unit_price: 500,
                                         status: 0)
    ...
    @razz_berry = @pokegarden.items.create!(name: 'Razz Berry',
                                            description: 'Feed this to a Pokémon, and it will be easier to catch on your next throw.', unit_price: 200)
    @nanab_berry = @pokegarden.items.create!(name: 'Nanab Berry',
                                             description: 'Feed this to a Pokémon to calm it down, making it less erratic.', unit_price: 500)
    @pinap_berry = @pokegarden.items.create!(name: 'Pinap Berry',
                                             description: 'Feed this to a Pokémon to make it drop more Candy.', unit_price: 800)
    ...
    @invoice_item3 = InvoiceItem.create!(invoice: @invoice3, item: @razz_berry, quantity: 2, unit_price: 800,
                                         status: 0)
    @invoice_item4 = InvoiceItem.create!(invoice: @invoice4, item: @pinap_berry, quantity: 2, unit_price: 500,
                                         status: 0)
